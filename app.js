...
async function submitForm(){
  ...
  // 🛡️ Vercel Secure Serverless Function Proxy Call [2]
...
// ─── HAMBURGER DRAW TOGGLE ENGINE [2] ────────────────────────────
...
// ─── MOBILE ZOOM BLOCKING TRIGGERS [2] ───────────────────────────
function disableMobileZoom() {
  document.addEventListener('touchstart', (e) => {
    if (e.touches.length > 1) e.preventDefault(); // ब्लॉक मल्टी-टच पिंच ज़ूम [2]
  }, { passive: false });

  let lastTouchEnd = 0;
  document.addEventListener('touchend', (e) => {
    const now = (new Date()).getTime();
    if (now - lastTouchEnd <= 300) e.preventDefault(); // ब्लॉक डबल-टैप ज़ूम [2]
    lastTouchEnd = now;
  }, false);
}
...
window.addEventListener('load',()=>{
  ...
  document.body.classList.remove('loading'); // लोडर हटते ही स्क्रॉलिंग अनलॉक [2]
  typeEffect();
  initReveal();
  renderSkills();
  renderProjects();
  renderFeatures();
  initFAQ();
  initHamburgerListener(); // हैमबर्गर एक्टिवेटर [2]
  disableMobileZoom();     // ज़ूम प्रिवेंटर [2]
